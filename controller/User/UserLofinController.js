      ...
      if (existingUser) {
        ...
        if (usereffral) {
          ...
          const phone = Jwt.sign(
            ...
            process.env.SECRETFORAUTH, // Change this to your secret key
...
exports.withdraw = async (req, res, next) => {
  try {
    ...
    if (existingUser) {
      ...
      return res
        .status(200)
        .json({ message: "Withdrawal data saved successfully" });
    } else {
...
exports.PlayasGuest = async (req, res, next) => {
  ...
  try {
    ...
    const token = Jwt.sign(
      { _id: savedUser._id },
      process.env.SECRETFORAUTH, // Change this to your secure secret key
      { expiresIn }
    );
    res.status(200).send({
...
exports.AddAdmin = async (req, res) => {
  try {
    await Admin_User.create(req.body).then(() => {
...
exports.UpdateAdmin = async (req, res) => {
  if (req.file) {
    ...
    const updatedAdmin = await Admin_User.findOneAndUpdate(
      {},
...
exports.ChangeAdminPassword = async (req, res) => {
  ...
  try {
    const updatedDocument = await Admin_User.findOneAndUpdate(
      { Password: password },
...
exports.DeleteAdminImg = async (req, res) => {
  ...
  try {
    const result = await Admin_User.updateOne(
      { Email: email },
...
exports.getdailyBonusDatas = async (req, res) => {
  try {
    ...
    const getDailyDaysDatas = getdailydata[0].DailyBonus;
    res.status(200).json(getDailyDaysDatas); // Assuming you want to send the data as a response
  } catch (error) {
